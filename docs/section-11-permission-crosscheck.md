...
| `POST_NOTIFICATIONS` | yes | the foreground-service notification on API 33+; requested when onboarding completes, on API 33+ — `completeOnboarding` (`src/permissions/sequencing.model.ts`) calls `requestNotifications` in `src/permissions/platform.gateway.ts`, which is a no-op below API 33 and off Android. It sits there rather than on the background step so that every path out of onboarding asks: "Not now", an already-granted background permission, a platform with no background step, and iOS | keep |
